...
public class MineSweeper extends JFrame implements KeyListener {
     ...
     * cell. If the current cell is not a mine ,and if it is already
     * marked sets it's color to default color.If the adjacent(not diagonals)
     * cell is safe it calls the mineCounter and passes the row and column of
     * the adjacent cell. If the current cell is mine it marks it red , disables
     * all the buttons and increments the number of lost games. If the game
     * hasWinner and the buttons are not disabled previously ,it disables all
     * the buttons and increment the number of won games
     */
    public void mineCounter(int x, int y) {
        ...
        if (!isMine(b[x][y])) {
            ...
            if (y > 0 && isMine(b[x][y - 1])) {
                mine++;
            } else if (y > 0 && isEnabled(b[x][y - 1]) && !isMine(b[x][y - 1])) {
                mineCounter(x, y - 1);
            }

            if (y < 7 && isMine(b[x][y + 1])) {
                mine++;
            } else if (y < 7 && isEnabled(b[x][y + 1]) && !isMine(b[x][y + 1])) {
                mineCounter(x, y + 1);
            }

            //number rof mines horizontaly
            if (x > 0 && isMine(b[x - 1][y])) {
                mine++;
            } else if (x > 0 && isEnabled(b[x - 1][y]) && !isMine(b[x - 1][y])) {
                mineCounter(x - 1, y);
            }
            if (x < 7 && isMine(b[x + 1][y])) {
                mine++;
            } else if (x < 7 && isEnabled(b[x + 1][y]) && !isMine(b[x + 1][y])) {
                mineCounter(x + 1, y);
            }
            //daigonals
